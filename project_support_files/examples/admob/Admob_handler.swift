...
class AdmobHandler: NSObject, AdmobHandler_Delegate {
   
    
   
    static let shared = AdmobHandler()
    
    private var py: AdmobHandlerPyCallback?
    ...
    private let adUnitID = "#adUnitID"
    
    override init() {
        super.init()
        InitAdmobHandler_Delegate(delegate: self)
        GADMobileAds.sharedInstance().start(completionHandler: nil)
        GADMobileAds.sharedInstance().requestConfiguration.testDeviceIdentifiers = [ "3657c067d510bcedb9e5605167f20c06" ]
    }
    
    func set_AdmobHandler_Callback(callback: AdmobHandlerPyCallback) {
        py = callback
    ...
    func static_banner_ad(enabled: Bool) {
        ...
        if !enabled {
            ...
            if staticBannerView == nil {
                ...
                staticBannerView.adUnitID = "#adUnitID"
...
extension AdmobHandler: GADBannerViewDelegate {
